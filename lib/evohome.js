...
const urlDomain = "https://mytotalconnectcomfort.com"
// 'https://tccna.honeywell.com';
const urlLogin = urlDomain + '/Auth/OAuth/Token';
const urlApiBase = urlDomain + '/WebAPI/emea/api/v1';
...
Session.prototype.getAllSchedules = async function() {
  // log.debug("/getAllSchedules: zone= " + util.inspect(this.locations[0].zones, {showHidden: false, depth: 1}))
  // log.debug("/getAllSchedules: typeof(Zones) = " + typeof(this.locations[0].zones))
  // log.debug("/getAllSchedules: length(Zones) = " + this.locations[0].zones.length)
  if (this.locations == null || this.locations.length <= 0 || this.locations[0].zones == null){
    log.debug("/getAllSchedules: locations[0].zones is null or zero length");
    return;
  }
...
Session.prototype.getScheduleForZoneId = async function(zoneId, zoneName) {
  ...
  var url = urlApiBase + "/temperatureZone/" + zoneId + "/schedule";
...
Session.prototype.getCurrentStatus = async function(locationId) {
  var url = urlApiBase + "/location/" + locationId + "/status?includeTemperatureControlSystems=True";
  log.silly("session.getCurrentStatus url: " + url);
  var json = await _request(url, this.credentials, 'GET');
  // log.debug("\n\n\nZone: " + JSON.stringify(this.locations[0].zones));
  // log.debug("\n\n\n");  
  
  // log.debug("this.locations[0].dhw: " + JSON.stringify(this.locations[0].dhw));
  // log.debug("\n\n\n");  

  var updatedZoneData = _.merge([],this.locations[0].zones, json.gateways[0].temperatureControlSystems[0].zones);
  // log.debug("\n\n\n");  
  // log.debug("updatedZoneData: " + JSON.stringify(updatedZoneData));
  // log.debug("\n\n\n");  
  ...
  // log.debug("updatedDhwData: " + JSON.stringify(updatedDhwData));
  // log.debug("\n\n\n");  
...
Session.prototype.saveScheduleForZone = async function(zone,zoneSchedule){
  ...
  var url = urlApiBase + "/temperatureZone/" + zone.zoneId + "/schedule";
...
Session.prototype.setZoneSetpoint = async function(zone,setpoint=0,until, locationIndex=0){ //Need to check timezone issues. "Until" time seems 4 hours ahead of whatever we set it to
  ...
  var url = urlApiBase + `/temperatureZone/${zoneId}/heatSetpoint`;
...
Session.prototype._setDhw = async function(body, locationIndex){
  ...
  var url = urlApiBase + `/domesticHotWater/${dhwId}/state`
...
Session.prototype.setSystemMode = async function(mode, until, locationIndex=0){
  ...
  var url = urlApiBase + `/temperatureControlSystem/${systemId}/mode`;    
  var options = {"body": JSON.stringify(body)};
  var response = await _request(url,this.credentials,'PUT',options);
...
async function getUserInfo (credentials) {
  var url = urlApiBase + '/userAccount';
...
async function getInstallationData (credentials, userId) {
  if (userId) {
    var url = urlApiBase + '/location/installationInfo?userId=' + userId + '&includeTemperatureControlSystems=True'
    var json = await _request(url, credentials, 'GET')
